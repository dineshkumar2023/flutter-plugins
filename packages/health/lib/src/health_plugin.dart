...
class Health {
  ...
  PlatformType get platformType => Platform.isAndroid ? PlatformType.ANDROID : PlatformType.IOS;
  ...
  bool isDataTypeAvailable(HealthDataType dataType) => platformType == PlatformType.ANDROID
  ...
  Future<bool> disconnect(
    ...
    return await _channel.invokeMethod('disconnect', {'types': keys, "permissions": mPermissions});
  }
  ...
  Future<List<HealthDataPoint>> _computeAndroidBMI(
    ...
    List<HealthDataPoint> heights =
        await _prepareQuery(startTime, endTime, HealthDataType.HEIGHT, includeManualEntry);
    ...
    List<HealthDataPoint> weights =
        await _prepareQuery(startTime, endTime, HealthDataType.WEIGHT, includeManualEntry);
  ...
  Future<List<HealthDataPoint>> getHealthIntervalDataFromTypes(
      DateTime startDate, DateTime endDate, List<HealthDataType> types, int interval,
      {bool includeManualEntry = true}) async {
    List<HealthDataPoint> dataPoints = [];

    for (var type in types) {
      final result =
          await _prepareIntervalQuery(startDate, endDate, type, interval, includeManualEntry);
      dataPoints.addAll(result);
    }
  ...
  Future<List<HealthDataPoint>> _prepareQuery(
    ...
    if (dataType == HealthDataType.BODY_MASS_INDEX && platformType == PlatformType.ANDROID) {
  ...
  /// Prepares an interval query, i.e. checks if the types are available, etc.
  Future<List<HealthDataPoint>> _prepareIntervalQuery(DateTime startDate, DateTime endDate,
      HealthDataType dataType, int interval, bool includeManualEntry) async {
    // Ask for device ID only once
    _deviceId ??= platformType == PlatformType.ANDROID
        ? (await _deviceInfo.androidInfo).id
        : (await _deviceInfo.iosInfo).identifierForVendor;
    ...
    return await _dataIntervalQuery(startDate, endDate, dataType, interval, includeManualEntry);
  }

  /// Prepares an aggregate query, i.e. checks if the types are available, etc.
  Future<List<HealthDataPoint>> _prepareAggregateQuery(DateTime startDate, DateTime endDate,
      List<HealthDataType> dataTypes, int activitySegmentDuration, bool includeManualEntry) async {
    // Ask for device ID only once
    _deviceId ??= platformType == PlatformType.ANDROID
        ? (await _deviceInfo.androidInfo).id
        : (await _deviceInfo.iosInfo).identifierForVendor;
    ...
    return await _dataAggregateQuery(
        startDate, endDate, dataTypes, activitySegmentDuration, includeManualEntry);
  }
  ...
  Future<List<HealthDataPoint>> _dataIntervalQuery(DateTime startDate, DateTime endDate,
      HealthDataType dataType, int interval, bool includeManualEntry) async {
    final args = <String, dynamic>{
      'dataTypeKey': dataType.name,
      'dataUnitKey': dataTypeToUnit[dataType].name,
  ...
  /// function for fetching statistic health data
  Future<List<HealthDataPoint>> _dataAggregateQuery(DateTime startDate, DateTime endDate,
      List<HealthDataType> dataTypes, int activitySegmentDuration, bool includeManualEntry) async {
    final args = <String, dynamic>{
  ...
  List<HealthDataPoint> _parse(Map<String, dynamic> message) {
    ...
    return dataPoints
        .map<HealthDataPoint>(
            (dataPoint) => HealthDataPoint.fromHealthDataPoint(dataType, dataPoint))
        .toList();
  }
  ...
  Future<bool> writeWorkoutData(
    ...
    } else if (platformType == PlatformType.ANDROID && !_isOnAndroid(activityType)) {
      throw HealthException(
